...
class RNNED(object):
    ...
    def __init__(self, nh, nc, de, model=None):
        ...
        if model is not None:
            ...
            [self.W_e, self.U_e, self.W_z_e, self.U_z_e, self.W_r_e, self.U_r_e, self.O_h, self.O_x, self.G, self.EMB,\
                self.EC, self.C_r_d, self.C_z_d, self.C_d, self.O_c] = [theano.shared(p) for p in model]
        else:
            ...
            #encoder matrix
            self.EC = theano.shared(name='EC',value=0.2 * numpy.random.uniform(-1.0, 1.0, (de, nc)).astype(theano.config.floatX))
            #context vector matrix c
        ...
        self.params = [self.W_e, self.U_e, self.W_z_e, self.U_z_e, self.W_r_e, self.U_r_e, self.O_h, self.O_x, self.G, self.EMB,\
                            self.EC, self.C_r_d, self.C_z_d, self.C_d, self.O_c]
        self.names = ['W_e', 'U_e', 'W_z_e', 'U_z_e', 'W_r_e', 'U_r_e', 'O_h', 'O_x', 'G', 'EMB', 'EC', 'C_r_d', 'C_z_d', 'C_d', 'O_c']
    ...
    def prepare_train(self):
        ...
        h_d_0 = self.h0
        def decoder(en, x, y_idx, train_flag=True):
            c = T.tanh(T.dot(self.EC, en))
            def decoder_recurrence(x_t, h_tm1, c):
                
                x_e = self.EMB[x_t,:]

                # Reset gate
                r = T.nnet.sigmoid(T.dot(self.W_r_e, x_e) + T.dot(self.U_r_e, h_tm1) + T.dot(self.C_r_d, c))
                # Update gate
                z = T.nnet.sigmoid(T.dot(self.W_z_e, x_e) + T.dot(self.U_z_e, h_tm1) + T.dot(self.C_z_d, c))
                # Gated output
                h_prime = T.tanh(T.dot(self.W_e, x_e) + r * (T.dot(self.U_e, r * h_tm1) + T.dot(self.C_d, c)))
                # Compute hidden state
                h_t = z * h_tm1 + (1 - z) * h_prime
                # Compute the final layer
                s = T.dot(self.O_h, h_t) + T.dot(self.O_x, x_e) + T.dot(self.O_c, c)
            ...
            # predict function
            self.predict = theano.function([en, x], p)
        ...
        phrase_train_nll, phrase_gradients = decoder(EN, X, Y_IDX)
        phrase_test_nll = decoder(EN, X, Y_IDX,False)
        ...
        self.phrase_train = theano.function(inputs=[EN,X,Y_IDX], on_unused_input='warn', updates=train_updates)
        # The test function return phrase average NLL
        self.phrase_test = theano.function(inputs=[i,EN,X,Y_IDX], on_unused_input='warn', updates=test_updates)



    def train(self, batch, lr):
        for (en,x,y_idx) in batch:
            self.phrase_train(en,x,y_idx)
        # Average gradients
        ...
        #update = rmsprop(self.params, grad_acc)
        update = adagrad(self.params, grad_acc)
        for (p,g) in update:
            p.set_value(g)
    ...
    def test(self, batch):
        ...
        for i, (en,x,y_idx) in enumerate(batch):
            self.phrase_test(i,en,x,y_idx,)
